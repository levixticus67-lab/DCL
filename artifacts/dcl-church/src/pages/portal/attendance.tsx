...
export default function AttendancePage() {
  ...
  return (
    <PortalLayout>
      <PortalHeader
        title="Attendance"
        subtitle="Track who's gathering each week"
        actions={
          editable ? (
            <Button
              onClick={openCreate}
        ...
        <div className="glass-strong rounded-2xl p-5">
          ...
          <div className="space-y-3 mt-4">
            {(summary?.byBranch ?? []).map((b) => (
              ...
              >
                <span className="text-[hsl(215,40%,30%)]">{b.branchName}</span>
                <span className="font-serif text-[hsl(215,80%,22%)] text-lg">
                  {b.total}
                </span>
              </div>
            ))}
        ...
        <Table>
          ...
          <TableBody>
            {services?.map((s) => {
              ...
              return (
                <TableRow key={s.id} data-testid={`row-att-${s.id}`}>
                  <TableCell className="whitespace-nowrap">
                    {formatDate(s.serviceDate)}
                  </TableCell>
                  <TableCell>{s.title}</TableCell>
                  <TableCell className="text-xs">{branch?.name ?? "—"}</TableCell>
                  <TableCell className="text-right">{s.adultCount}</TableCell>
                  <TableCell className="text-right">{s.youthCount}</TableCell>
                  <TableCell className="text-right">{s.childrenCount}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => openEdit(s)}
                          data-testid={`button-edit-att-${s.id}`}
                        >
                        ...
                        <Button
                          ...
                          data-testid={`button-delete-att-${s.id}`}
                        >
        ...
        <DialogContent className="glass-strong border-white/60">
          <DialogHeader>
            <DialogTitle className="font-serif">
              {editingId ? "Edit service" : "Record service"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <Field label="Branch">
              ...
              >
                <SelectTrigger data-testid="select-att-branch">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="grid grid-cols-3 gap-3">
              <Field label="Adults">
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, adultCount: e.target.value })
                  }
                  data-testid="input-att-adults"
                />
              </Field>
              <Field label="Youth">
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, youthCount: e.target.value })
                  }
                  data-testid="input-att-youth"
                />
              </Field>
              <Field label="Children">
                <Input
                  ...
                  data-testid="input-att-children"
                />
              </Field>
            </div>
            <Field label="Notes">
